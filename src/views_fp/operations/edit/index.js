...
const UserEdit = () => {
  ...
  const [activeTab, setActiveTab] = useState('1'),
    store = useSelector(state => state.operations),
  ...
  useEffect(() => {
    dispatch(getOperation(parseInt(id)))

    console.log('operation store : ', store)
   
  }, [dispatch])

  return store.selectedOperation !== null && store.selectedOperation !== undefined ? (
        ...
        <Card>
          <CardBody className='pt-2'>
            <Nav pills>
              <NavItem>
                <NavLink active={activeTab === '1'} onClick={() => toggle('1')}>
                  <User size={14} />
                  <span className='align-middle d-none d-sm-block'>Opération</span>
                </NavLink>
              </NavItem>
              {/* <NavItem>
            ...
            <TabContent activeTab={activeTab}>
              <TabPane tabId='1'>
                <OperationTab selectedOperation={store.selectedOperation} />
              </TabPane>
             {/*   <TabPane tabId='2'>
                <InfoTab />
              </TabPane>
